...
export default function NotFound() {
  ...
  return (
    <div className="mx-auto max-w-[var(--container-default)] px-[clamp(16px,4vw,32px)] py-[var(--spacing-32)]">
      ...
      <p className="mt-[var(--spacing-4)] text-base text-[var(--color-fg-subtle)]">
        {copy.h1Es.split("· ")[1]}
      </p>

      <p className="mt-[var(--spacing-6)] max-w-[600px] text-lg leading-[var(--leading-relaxed)] text-[var(--color-fg-muted)]">
        {copy.bodyEn}
      </p>
      <p className="mt-[var(--spacing-2)] max-w-[600px] text-sm leading-[var(--leading-relaxed)] text-[var(--color-fg-subtle)]">
        {copy.bodyEs}
      </p>
      ...
      <ul className="mt-[var(--spacing-6)] grid gap-[var(--spacing-4)] md:grid-cols-2">
        {trackA.map((project) => (
          <li key={project.slug}>
            ...
            >
              <p className="text-sm font-bold uppercase tracking-[var(--tracking-widest)]">
                {project.name}
              </p>
      ...
      <div className="mt-[var(--spacing-12)] flex flex-wrap gap-[var(--spacing-4)]">
        <Link
          href="/en"
          className="border-2 border-[var(--color-fg)] px-[var(--spacing-6)] py-[var(--spacing-3)] font-bold uppercase tracking-[var(--tracking-widest)] text-xs text-[var(--color-fg)] transition-all duration-[var(--duration-instant)] ease-[var(--ease-snap)] hover:border-[var(--color-accent)] hover:bg-[var(--color-accent-glow)] hover:text-[var(--color-accent)]"
        >
          [ ← {copy.backHome} ]
        </Link>
      </div>
